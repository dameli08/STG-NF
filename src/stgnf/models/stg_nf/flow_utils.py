"""Small tensor helpers shared by the flow modules (faithful port of
...
def split_feature(tensor, type="split", imgs=False):
    """Split a tensor along the channel dim.

    ``type='split'`` -> first/second halves; ``type='cross'`` -> even/odd channels.
    """
    C = tensor.size(1)
    if imgs:
        if type == "split":
