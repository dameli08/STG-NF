"""Glow-style flow modules for STG-NF.

Faithful port of the official ``models/STG_NF/modules_pose.py`` (based on
https://github.com/y0ast/Glow-PyTorch). The only change is modernizing the
deprecated ``torch.lu`` / ``torch.lu_unpack`` LU factorization to
``torch.linalg.lu_factor`` in :class:`InvertibleConv1x1`.
"""
import math
...
from stgnf.models.stg_nf.flow_utils import split_feature, compute_same_pad


def gaussian_p(mean, logs, x):
    """log N(x; mean, exp(logs)^2), elementwise (diagonal, independent)."""
...
class InvertibleConv1x1(nn.Module):
    def __init__(self, num_channels, LU_decomposed):
        ...
        if not LU_decomposed:
            ...
            # Modernized: torch.lu/torch.lu_unpack -> torch.linalg.lu_factor.
